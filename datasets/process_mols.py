...
from utils.metal_properties import MetalProperties
...
###  featurization for metal ions
def metal_featurizer(mol):
    metal_props = MetalProperties()
    atom_features_list = []
    for atom in mol.GetAtoms():
        atom_name = atom.GetSymbol()
        normalized_features_dict = metal_props.get_normalized_metal_features(atom_name)
        normalized_features_list = list(normalized_features_dict.values())
        atom_features_list.append(normalized_features_list)
    return torch.tensor(atom_features_list)
###
...
def get_lig_graph(mol, complex_graph, use_metal_featurizer):
    lig_coords = torch.from_numpy(mol.GetConformer().GetPositions()).float()
    if use_metal_featurizer:
        atom_feats = metal_featurizer(mol)
    else:
...
def get_lig_graph_with_matching(mol_, complex_graph, popsize, maxiter, matching, keep_original, num_conformers, remove_hs, use_metal_featurizer):
    if matching:
        ...
        get_lig_graph(mol_, complex_graph, use_metal_featurizer)
