...
class ConfidenceDataset(Dataset):
    ...
    def preprocessing(self, original_model_cache):
        ...
        for idx, orig_complex_graph in tqdm(enumerate(self.loader)):
            ...
            # ##
            num_metal = 100
            randomize_position_multiple(data_list, False, self.original_model_args.tr_sigma_max, metal_num=num_metal)
            # larger_original_graph(orig_complex_graph, self.original_model_args.tr_sigma_max, metal_num=num_metal)
            ...
            ligand_pos = np.asarray([complex_graph['ligand'].pos.cpu().numpy() for complex_graph in predictions_list])
            orig_ligand_pos = np.expand_dims(orig_complex_graph['ligand'].orig_pos - orig_complex_graph.original_center.cpu().numpy(), axis=0)
            
            ## recacluateing rmsds to consider original pdb file
            real_zinc_pos = find_real_zinc_pos(os.path.join(self.args.data_dir, f"{orig_complex_graph.name[0]}/{orig_complex_graph.name[0]}_ligands.mol2"))
            positions_new = ligand_pos.squeeze(0) + orig_complex_graph.original_center.cpu().numpy()
            rmsd, indices = get_nearest_point_distances(positions_new, real_zinc_pos)
            ##
