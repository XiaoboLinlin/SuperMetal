import numpy as np
import warnings
from Bio.PDB import PDBParser
from openbabel import openbabel as ob

def find_real_zinc_pos(file_path, model_index=0):
    """
    Load a PDB or MOL2 file and extract positions of zinc atoms using Bio.PDB or Open Babel.
    
    :param file_path: Path to the file.
    :param model_index: Index of the model from which to extract zinc positions for PDB files (default is 0).
    """
    file_extension = file_path.split('.')[-1].lower()
    
    if file_extension == 'pdb':
        warnings.simplefilter('ignore')  # General suppression of warnings
        
        # Initialize the PDB parser
        parser = PDBParser(QUIET=True)
        
        # Parse the structure from the file
        structure = parser.get_structure('PDB_structure', file_path)
        
        # Initialize a list to store zinc positions
        zinc_positions = []
        
        # Iterate only over the first model
        first_model = next(structure.get_models())
        for chain in first_model:
            for residue in chain:
                for atom in residue:
                    if atom.element == 'ZN':
                        # Append the atom coordinates to the list
                        zinc_positions.append(atom.coord)
        
                        
    elif file_extension == 'mol2':
        obConversion = ob.OBConversion()
        obConversion.SetInFormat("mol2")
        mol = ob.OBMol()
        obConversion.ReadFile(mol, file_path)
        zinc_positions = []
        for atom in ob.OBMolAtomIter(mol):
            if atom.GetType() == 'Zn':
                zinc_positions.append(np.array([atom.GetX(), atom.GetY(), atom.GetZ()]))

    else:
        raise ValueError("Unsupported file format. Please provide a PDB or MOL2 file.")
        
    return np.array(zinc_positions)
